...
if docker images --format "{{.Repository}}" | grep -q "gcr.io/networklogic01/frontend-image"; then
    # If the image exists, remove it
    docker rmi gcr.io/networklogic01/frontend-image
fi

pwd
cd ..
# Change directory to the parent folder of frontend and build the Docker image
docker build -f frontend.Dockerfile -t gcr.io/networklogic01/frontend-image .

# Push the Docker image to Google Container Registry
docker push gcr.io/networklogic01/frontend-image:latest
